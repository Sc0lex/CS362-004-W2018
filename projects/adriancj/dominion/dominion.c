...
// The current player draws three additional cards from their pile, 
// then discards their smithy card
int smithyEffect(struct gameState *state, int currentPlayer, int handPos) {
	int i;
	//+3 Cards *BUG - actually draws 4
	for (i = 0; i <= 3; i++)
	{
		drawCard(currentPlayer, state);
	}
...
// The player draws cards until they get two treasure cards. The treasure cards are added
// to the deck and the other drawn cards are discarded.
...
// The feast card gets trashed and the purchased card goes into discard pile
int feastEffect(struct gameState *state, int currentPlayer, int choice) {
	...
	//Backup hand *BUG - removed curly braces
	for (i = 0; i <= state->handCount[currentPlayer]; i++)//{
...
// The player chooses either to add two cards to their hand, add two coin
// toward their buy or trash two cards from their hand.
int stewardEffect(struct gameState *state, 
					  int currentPlayer, 
					  int handPos,
					  int optionChoice, 
					  int trashChoice1, 
	...
	{
		//trash 2 cards in hand *BUG - cleared trash flag
		discardCard(trashChoice1, currentPlayer, state, /*1*/0);
		discardCard(trashChoice2, currentPlayer, state, /*1*/0);
	}
...
// Gives one additional buy and allows the player to choose between discarding an estate
// in exchange for 4 coin or gaining an estate
int baronEffect(struct gameState *state, int currentPlayer, int optionChoice) {
	state->numBuys++;//Increase buys by 1!
	if (optionChoice > 0){//Boolean true or going to discard an estate
